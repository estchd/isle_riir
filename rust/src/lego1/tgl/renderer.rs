...
pub trait Renderer: Object {
	...
	fn create_light(&mut self,
	                light_type: LightType,
	                r: f32,
	                g: f32,
	                b: f32
	) -> Option<Box<dyn Light>>;

	fn create_group(&mut self, parent: &dyn Group) -> Option<Box<dyn Group>>;

	fn create_mesh_builder(&mut self) -> Option<Box<dyn MeshBuilder>>;

	fn create_texture_with_params(&mut self,
	                              width: i32,
	                              height: i32,
	                              bits_per_texel: i32,
	                              texels: &[u8],
	                              texels_are_persistent: i32,
	                              palette_entry_count: i32,
